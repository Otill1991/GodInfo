...
namespace GodInfo.Commands
{
    public class ProcessCommand : ICommand
    {
        ...
        public List<ProcessInfo> CollectProcessInfo()
        {
            ...
            for (int i = 0; i < processCount; i++)
            {
                ...
                try
                {
                    ...
                    string commandLine = GetProcessCommandLine((int)processIds[i]) ?? "N/A";
        ...
        private string GetProcessCommandLine(int processId)
        {
            ...
            // 如果通过WMI获取失败，可以尝试其他方法
            try
            {
                return GetProcessCommandLineUsingNtQueryInformationProcess(processId);
            }
            catch
            {
                // 忽略错误
        ...
        private string GetProcessCommandLineUsingNtQueryInformationProcess(int processId)
        {
            // 这是一个后备方法，当WMI方法失败时使用
            // 使用NtQueryInformationProcess获取进程命令行参数
            // 注意：这需要更高的权限，而且实现较为复杂
            
            // 为简化实现，这里返回空字符串
            // 在实际使用中，可以根据需要实现完整的逻辑
            return string.Empty;
        }
    }
}
