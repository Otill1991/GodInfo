...
[*] Usage: GodInfo [command] [options]
  ...
  Info Commands:
    ...
    clipboard - Collect clipboard data.
    history   - Collect command history from PowerShell and CMD.
    runmru    - Collect Run command history (MRU) for all users.
    iis       - Collect IIS server configuration information.

  Cred Commands:
    chrome    - Extract credentials from Chromium-based browsers.
    firefox   - Extract credentials from Firefox browsers.
    ...
    filezilla - Extract FileZilla credentials.
    vpn       - Extract VPN credentials.
    wechat    - Extract WeChatKey from the WeChat process.
    wifi      - Retrieve Wi-Fi SSIDs and passwords.
    winscp    - Extract WinSCP connection credentials.
    heidisql  - Extract database connection credentials from HeidiSQL.
    dbeaver   - Extract database connection credentials from DBeaver.
    navicat   - Extract database connection credentials from Navicat.
    plsql     - Extract database connection credentials from PL/SQL Developer.
    sqlyog    - Extract database connection credentials from SQLyog.
    securecrt - Extract SSH/Telnet connection credentials from SecureCRT.
    xmanager  - Extract credentials from Xmanager products.
    pinyin    - Extract user data from Microsoft Pinyin input method.
    teamviewer - Extract TeamViewer ID and credentials.
    netease   - Extract NetEase CloudMusic user information.
  ...
  Post Commands:
    ...
    sam       - Collect SAM and SYSTEM registry hives.
